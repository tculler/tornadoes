...
# add_density_overlay
# ---------------------------------------------------------------------------
class TestAddDensityOverlay:
    @staticmethod
    def _sample_dataframe() -> "pd.DataFrame":
        """Create a sample DataFrame with tornado data for overlay testing."""
        return pd.DataFrame(
            {
                "BEGIN_LAT": [40.0, 40.1, 40.2, 50.0, 50.1],
                "BEGIN_LON": [-85.0, -85.1, -85.2, -75.0, -75.1],
                "SEASON": ["Spring", "Spring", "Spring", "Summer", "Summer"],
                "INTENSITY_NORM": ["EF3", "EF3", "EF4", "EF4", "EF5"],
                "COUNTRY": ["US", "US", "US", "US", "US"],
                "ADMIN_AREA": ["Michigan", "Michigan", "Michigan", "New York", "New York"],
            }
        )

    def test_overlay_ignores_none_folium(self, monkeypatch):
        """Test that overlay gracefully handles None folium."""
        monkeypatch.setattr(app, "folium", None)
        df = self._sample_dataframe()
        fmap_mock = MagicMock()
        # Should return early without error
        app.add_density_overlay(fmap_mock, df, "SEASON")

    def test_overlay_returns_on_invalid_group_column(self):
        """Test that overlay returns if group_by column doesn't exist."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = self._sample_dataframe()
        fmap_mock = MagicMock()
        # Should return early without error
        app.add_density_overlay(fmap_mock, df, "NONEXISTENT_COLUMN")

    def test_overlay_skips_groups_below_minimum_points(self):
        """Test that groups with fewer than min_points are skipped."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = pd.DataFrame(
            {
                "BEGIN_LAT": [40.0, 50.0],
                "BEGIN_LON": [-85.0, -75.0],
                "SEASON": ["Spring", "Winter"],  # Each has only 1 point
            }
        )
        fmap_mock = MagicMock()
        # With min_points=3, both groups should be skipped
        app.add_density_overlay(fmap_mock, df, "SEASON", min_points=3)
        # No heatmap layers should have been added
        # (fmap_mock.add_child won't be called or HeatMap.add_to)

    def test_overlay_processes_groups_above_minimum_points(self):
        """Test that groups with >= min_points are processed."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = self._sample_dataframe()
        fmap_mock = MagicMock()
        # Should process groups with 3+ points
        app.add_density_overlay(fmap_mock, df, "SEASON", min_points=3)
        # Function should complete without error

    def test_overlay_handles_nans_in_coordinates(self):
        """Test that overlay gracefully handles NaN coordinates."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = pd.DataFrame(
            {
                "BEGIN_LAT": [40.0, None, 40.2, 40.3, 40.4],
                "BEGIN_LON": [-85.0, -85.1, None, -85.3, -85.4],
                "SEASON": ["Spring", "Spring", "Spring", "Spring", "Spring"],
            }
        )
        fmap_mock = MagicMock()
        # Should skip NaN rows and process valid coordinates
        app.add_density_overlay(fmap_mock, df, "SEASON", min_points=3)

    def test_overlay_groups_by_intensity(self):
        """Test overlay grouping by intensity."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = self._sample_dataframe()
        fmap_mock = MagicMock()
        # Should group by INTENSITY_NORM and create layers for each
        app.add_density_overlay(fmap_mock, df, "INTENSITY_NORM", min_points=2)

    def test_overlay_groups_by_country(self):
        """Test overlay grouping by country."""
        if app.folium is None:
            pytest.skip("folium not installed")
        df = self._sample_dataframe()
        fmap_mock = MagicMock()
        # Should group by COUNTRY
        app.add_density_overlay(fmap_mock, df, "COUNTRY", min_points=3)
